...
QUnit.test("isosceles triangle has two equal sides", function( assert ) {
	...
	assert.equal(triangle.isIsosceles(1,1,2),false,"1,1,2 is not") ;
	assert.equal(triangle.isIsosceles(2,2,6),false,"2,2,6 is not") ;
});
QUnit.test("scalene triangle has three different sides", function( assert ) {
	...
	assert.equal(triangle.isScalene(1,2,3),false,"1,2,3 is not") ;
